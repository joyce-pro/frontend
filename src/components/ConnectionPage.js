...
function ConnectionResultCard({ connectionid, connection_name }) {
    const navigate = useNavigate();
    const [showPopup, setShowPopup] = useState(false);

    const handleNavigate = () => {
        navigate(`/connection/${connectionid}`); // Navigate to the path "/fetch/:id"
    };

    const baseUrl = process.env.REACT_APP_API_URL;

    const sendMessage = async (connectionId, event) => {
        event.stopPropagation(); // Prevent click event from triggering navigation

        try {
            const response = await fetch(`${baseUrl}/connection/send`, {
                method: "POST",
                headers: {
    ...
    return (
        ...
        >
            <h5 className="text-primary text-2xl font-bold mb-4">{connection_name}</h5>

            {/* Play Button - Prevents Navigation */}
            <button
                onClick={(e) => sendMessage(connectionid, e)}
                ...
                <div className="container mx-auto px-4" data-aos="fade-up">
                    <div className="text-center">
                        <div className="relative mb-6">
                            <h2 className="text-3xl font-bold text-center">Connection Campaigns</h2>
                            <button
                            ...
                            >
                                Create New Connection Campaign
                            </button>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                            {results && results.length > 0 ? (
                                results.map((result, index) => (
                                    <ConnectionResultCard key={index} {...result} />
                                ))
                            ) : (
