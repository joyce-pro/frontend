...
export default function ConnectionResults() {
    ...
    useEffect(() => {
        const handleSearch = async () => {
            ...
            try {
                const response = await fetch(`${baseUrl}/connection/fetch/${id}`);
                if (!response.ok) {
                    throw new Error(`HTTP error! Status: ${response.status}`);
                }
                const data = await response.json();
                setResults(data.connection);
            } catch (err) {
                console.log(err);
            }
        };
    ...
    const handleRemoveFromQueue = (userurl) => {
        setResults((prevQueue) => prevQueue.filter((item) => item.userurl !== userurl));
    };
            ...
            <section id="search-results" className="pt-7 mt-20 bg-primary text-gray-900 relative">
                <div className="container mx-auto px-4" data-aos="fade-up">
                    <div className="relative mb-6">
                        <h2 className="text-3xl font-bold text-center">Connection Campaign Results</h2>
                        {/* <button
