...
import WhatsappOverlay from "./WhatsappOverlay";
const { DateTime } = require('luxon');

function WhatsappResultCard({ whatsappid, whatsapp_name }) {
    const navigate = useNavigate();
    const [showPopup, setShowPopup] = useState(false);

    const handleNavigate = () => {
        navigate(`/whatsapp/${whatsappid}`); // Navigate to the path "/fetch/:id"
    };

    const baseUrl = process.env.REACT_APP_API_URL;

    const sendMessage = async (whatsappid, event) => {
    ...
    return (
        ...
        >
            <h5 className="text-primary text-2xl font-bold mb-4">{whatsapp_name}</h5>

            {/* Play Button - Prevents Navigation */}
            <button
                onClick={(e) => sendMessage(whatsappid, e)}
                ...
                <div className="container mx-auto px-4" data-aos="fade-up">
                    <div className="text-center">
                        <div className="relative mb-6">
                            <h2 className="text-3xl font-bold text-center">Whatsapp Campaigns</h2>
                            <button
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                            {results && results.length > 0 ? (
                                results.map((result, index) => (
                                    <WhatsappResultCard key={index} {...result} />
                                ))
                            ) : (
